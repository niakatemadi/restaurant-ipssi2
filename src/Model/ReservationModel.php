...
class ReservationModel extends Database{
    ...
    public function getAvailableTable()
    {
        ...
        if(!empty($_GET['debut_reservation']) && !empty($_GET['fin_reservation']))
        {
            ...
            $stmt = "SELECT DISTINCT(tables.nom) FROM tables LEFT JOIN reservations ON tables.id = reservations.id_table WHERE (reservations.id IS NULL OR reservations.debut_reservation > '2022-04-10 17:10:00' OR reservations.fin_reservation < '2022-04-10 13:00:00')
            AND tables.id NOT IN (SELECT tables.id FROM tables LEFT JOIN reservations ON tables.id = reservations.id_table WHERE reservations.debut_reservation < '2022-04-10 17:00:00' AND reservations.fin_reservation > '2022-04-10 13:00:00')
            ";
